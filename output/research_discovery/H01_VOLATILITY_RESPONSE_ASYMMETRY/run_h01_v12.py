...
# -*- coding: utf-8 -*-
"""
QUANTFORGE — H01 VOLATILITY RESPONSE ASYMMETRY V1.2 — CONTROLLED RE-EXECUTION
Protocol: output/research_discovery/H01_VOLATILITY_RESPONSE_ASYMMETRY_PROTOCOL_V1.md (v1.2.0, FROZEN)
v1.2 changes ONLY the null-inference layer (protocol §9A/§16): the null-imposing data-level
construction replaces the retired v1.1 p-value. All scientific objects, matching, universes,
strata, bootstrap machinery, Holm family and secondaries are identical to the v1.1 execution.
H01 v1.1 remains CONFIRMATORY INFERENCE INVALID / SCIENTIFIC RESULT UNADJUDICATED; no v1.1
p-values are used or mixed with v1.2 results. Artifacts are written under
output/research_discovery/H01_VOLATILITY_RESPONSE_ASYMMETRY/ with V1.2-suffixed names.
...
FROZEN_PARAMS = {
    ...
    'bootstrap': {'L': L_BLOCK, 'B': B_REPL, 'seed': SEED,
                  ...
                  'p': 'p = (1 + count)/(B + 1), count = #{b : |D*_null,b| >= |D_obs|} (null-test, v1.2.0)',
                  'ci': 'percentile (2.5%, 97.5%) of the ordinary sampling draws D*'},
    'null_construction': {'form': 'data-level, authoritative (protocol §9A.2)',
                          'constant': 'c_(m,s) = mean(d_i) over the frozen observed matched pairs in (m,s)',
                          'transformation': 'shock-response level: dlnRV(neg) - c_(m,s)/2; dlnRV(pos) + c_(m,s)/2 (any equivalent sign-split producing d_i - c_(m,s))',
                          'pivot': 'D*_null = D* - D_obs only where exactly equivalent (all 3 strata present); never silently substituted',
                          'mc_convention': '(1 + count)/(B + 1), count = #{b : |D*_null,b| >= |D_obs|}'},
...
def rep_class_dm2(pack, starts):
    """Per-replicate draws (D_class^b sampling, D_class^b null) given the frozen class pack
    and the sampled circular-block start positions (protocol §9.3/§9.4, §9A.2).

    The null draw is the data-level construction computed on the SAME replicate pairs
    (matching is response-blind): each pair's null difference = raw difference - c_(m,s),
    which is mathematically identical to the registered shock-response ±c_(m,s)/2
    transformation (protocol §9A.2). Empty-stratum handling is exactly the registered
    §9.3 rule (mean over strata yielding >= 1 pair), so the data-level form is authoritative
    and the statistic-level shortcut D*_null = D* - D_obs is never substituted."""
    ...
    for m in pack['markets']:
        ...
        for s in range(3):
            ...
            if len(diffs) > 0:
                ...
                # data-level null: pair difference - c_(m,s); c_ms is frozen and finite for
                # every evaluable market (pack-build gate in main())
...
def worker_rep(task):
    key, b, starts = task
    dm, dn = rep_class_dm2(_WORKER_PACKS[key], starts)
    return key, b, dm, dn
...
def main():
    ...
    for cd in class_defs:
        ...
        ev = [m for m in cd['markets'] if per_market[m]['evaluable']]
        if not ev:
            bootstrap[key] = {'T': None, 'nblocks': None, 'p_null': math.nan,
                              'ci95': [math.nan, math.nan], 'n_draws': 0,
                              'null_mean': None, 'sampling_mean': None,
                              'class_draws': [], 'class_null_draws': []}
            continue
        union_dates = set()
        for m in cd['markets']:
        ...
        for m in ev:
            rec = repl[m]
            c_ms = per_market[m]['c_stratum']
            if any(c is None for c in c_ms.values()):
                raise SystemExit(f'STOP (protocol §23): evaluable market {m} has an undefined null constant c_(m,s)')
            pm[m] = {k: rec[k] for k in ('mag', 'resp', 'stratum', 'neg', 'tind', 'calipers')}
            pm[m]['c_ms'] = {s: float(c_ms[s]) for s in range(3)}
    ...
    def _cache_paths(key):
        return (os.path.join(CACHE_DIR, f'bootstrap_H01_V12_{key}.npy'),
                os.path.join(CACHE_DIR, f'bootstrap_H01_V12_{key}.json'))
    ...
    def _save_cache(key, draws, n_consumed):
        ...
        tmp = npy + '.tmp'
        # retry the open itself: transient WinError 13 (AV / file-lock contention) has
        # been observed on the tmp file on this platform; retrying keeps progress safe
        for i in range(12):
            try:
    ...
    for key in order:
        ...
        if cached is not None and cached.shape == (B_REPL, 2):
            draws_all[key] = cached
            nc0[key] = nc
        else:
            draws_all[key] = np.full((B_REPL, 2), math.nan)
            nc0[key] = 0
    ...
    def _run_serial():
        for key, b, starts in _task_stream():
            dm, dn = rep_class_dm2(packs[key], starts)
            draws_all[key][b, 0] = dm
            draws_all[key][b, 1] = dn
            progress[key] = b + 1
    ...
    if workers > 1:
        ...
        with ProcessPoolExecutor(max_workers=workers, initializer=init_worker,
            ...
            for key, b, dm, dn in ex.map(worker_rep, _task_stream()):
                draws_all[key][b, 0] = dm
                draws_all[key][b, 1] = dn
                progress[key] = b + 1
                if (b + 1) % 50 == 0 or (b + 1) == B_REPL:
    ...
    for key in order:
        ...
        if obs is not None and finite_null.size:
            ...
            p = math.nan
        if finite_raw.size:
            ci = [float(np.percentile(finite_raw, 2.5)), float(np.percentile(finite_raw, 97.5))]
        else:
        ...
        bootstrap[key] = {'T': packs[key]['T'], 'nblocks': packs[key]['nblocks'],
                          ...
                          'class_draws': raw.tolist(), 'class_null_draws': null.tolist()}
        if finite_null.size:
            print(f'  {key}: p_null={p:.5f} CI={ci[0]:.5f}..{ci[1]:.5f} '
                  f'null_mean={finite_null.mean():+.5f}', flush=True)
        else:
            print(f'  {key}: p_null=nan', flush=True)

    # ----- Holm
    fam_order = sorted(class_results.keys())
    raw_ps = {k: bootstrap[k]['p_null'] for k in fam_order}
    ...
    with open(os.path.join(OUT, 'matched_pairs_H01_V1.2.csv'), 'w', newline='', encoding=ENCODING) as f:
        w = csv.writer(f)
    ...
    with open(os.path.join(OUT, 'per_market_stats_H01_V1.2.csv'), 'w', newline='', encoding=ENCODING) as f:
        w = csv.writer(f)
    ...
    with open(os.path.join(OUT, 'bootstrap_replicates_H01_V1.2.csv'), 'w', newline='', encoding=ENCODING) as f:
        w = csv.writer(f)
        w.writerow(['class_layer', 'replicate', 'D_class_star', 'D_class_null'])
        for k in fam_order:
            dr = bootstrap[k]['class_draws']
            dn = bootstrap[k]['class_null_draws']
            for i, (v, vn) in enumerate(zip(dr, dn), start=1):
                w.writerow([k, i,
                            '' if v is None or (isinstance(v, float) and math.isnan(v)) else repr(v),
                            '' if vn is None or (isinstance(vn, float) and math.isnan(vn)) else repr(vn)])
    ...
    results = {
        ...
        'bootstrap': {
            k: {'T': v['T'], 'nblocks': v['nblocks'], 'p_null': v['p_null'], 'ci95': v['ci95'],
                'n_draws': v['n_draws'], 'null_mean': v['null_mean'], 'sampling_mean': v['sampling_mean']}
        ...
        'notes': {
            ...
            'v1.2_null_inference': 'only the null-testing/p-value layer changed (protocol §9A/§16 v1.2.0); '
                                   'null draws D*_null from the data-level construction (pair difference - c_(m,s), '
                                   'mathematically identical to the registered shock-response ±c_(m,s)/2 transformation); '
                                   'c_(m,s) frozen from the original matched pairs; p_null = (1+count)/(B+1), '
                                   'count = #{b : |D*_null,b| >= |D_obs|}; percentile CI from the ordinary sampling draws D*',
            'v1.1_status': 'H01 v1.1 execution remains CONFIRMATORY INFERENCE INVALID / SCIENTIFIC RESULT UNADJUDICATED; '
                           'no v1.1 p-values are reused or mixed with v1.2 results',
        },
    }
    with open(os.path.join(OUT, 'results_H01_V1.2.json'), 'w', encoding=ENCODING) as f:
        json.dump(results, f, indent=2, default=str)

    meta = {
        'experiment': 'H01 VOLATILITY RESPONSE ASYMMETRY V1.2 (CONTROLLED RE-EXECUTION)',
        'protocol_version': PROTOCOL_VERSION,
        'protocol_sha256': fp['protocol_sha256'],
        'source_sha256': {
        ...
        'p_null_convention': '(1+count)/(B+1), count = #{b : |D*_null,b| >= |D_obs|}; D*_null from the data-level null construction (protocol §9A.2)',
        'executed_at': results['executed_at'],
        'note': 'Controlled re-execution of frozen protocol v1.2.0; only the null-inference layer changed (protocol §9A/§16); no outcome-driven tuning.',
    }
    with open(os.path.join(OUT, 'experiment_metadata_H01_V1.2.json'), 'w', encoding=ENCODING) as f:
